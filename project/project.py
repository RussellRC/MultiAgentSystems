...
class QuotingAgentOutput(BaseModel):
    ...
    """
    quote_calculation: QuoteCalculation | None = Field(description="Detailed field-by-field breakdown of the quote calculation.")
    quote_explanation: str | None = Field(description="Human-readable explanation of the quote, including base cost, discounts, and estimated delivery date.")
    estimated_delivery_date: str | None = Field(description="Estimated delivery date for the order in ISO format")
    messages: List[str] = Field(description="List of all answer messages in response to the given request.", default_factory=list)
...
QUOTING_AGENT_SYSTEM_PROMPT = (
    ...
    "6. Get the unit price of the item using the `get_inventory_items_by_name` tool. If it doesn't yield any result, then use the best fit from the analysis you did from the historical quote search in the previous step.\n"
...
class RequestMetadata(BaseModel):
    ...
    mood: str = Field(description="Likely mood or tone of the customer.", default="neutral", examples=["happy", "angry", "sad", "miserable", "pissed off", "stressed", "neutral"], alias="customer_mood")
    job: str = Field(description="Possible job or occupation of the customer.", default="unknown", examples=["event manager", "restaurant manager", "director", "city hall clerk"], alias="job_type")
    need_size: str = Field(description="Qualitative size of the order.", default="medium", examples=["small", "medium", "large"], alias="order_size")
    event: str = Field(description="Event for which the customer wants the order.", default="unknown", examples=["unknown", "meeting", "festival", "concert", "ceremony", "party", "conference", "gathering"], alias="event_type")
...
class CustomerRequestDetails(BaseModel):
    ...
    delivery_date: str | None = Field(description="Expected delivery date for the order, in ISO format.", examples=["2026-01-01"])
...
ORDER_PROCESSOR_INSTRUCTIONS = (
    "- Use your tools to help you identify the item names that exist (are known) in the company's inventory.\n"
    "- For clarity, an item is a catalog product name only. Numbers and units of measure such as sheets, reams, packs, boxes, rolls, etc., are part of the quantity, not separate items.\n"
    "- If an item name from the user input has many potential matches, break the tie using the most expensive one,"
    " e.g., 'A4 glossy paper' can match both 'Glossy paper' and 'A4 paper', but Glossy is more expensive, so use this one.\n"
    "- If a requested item is not present in the company's item names catalog, accept the request if it's relevant to the company's business.\n"
    "- The possible reasons for you to decline a request are:\n"
    "   - You were not able to understand it\n"
    "   - You were not able to identify the items and quantities\n"
    "   - The customer is asking for items irrelevant to a paper supplies company, e.g. firearms, sporting goods, food, etc.\n"
...
class OrchestratorAgentOutput(BaseModel):
    ...
    """
    customer_response: str = Field(description=("Human-readable response for the customer. Must be in plain text, informing him/her of the order status, total price, and delivery dates. "
                                                "It should also have confirmation about the quantities, quoted prices and applied discounts."))
    total_amount: float = Field(description="Grand total price of the order quoted to the customer, including discounts. Computed by the sum of all the individual quotes for each item in the order.")
...
ORCHESTRATOR_AGENT_SYSTEM_PROMPT = (
    ...
    "3. For each item-quantity pair identified in the customer request details do:\n"
    "   - Call the `generate_quote` tool to get a quote from the Quoting Agent. Don't forget to provide the requested delivery date.\n"
    "   - Check the messages from the response, if the Quoting Agent is unable to generate a quote, then answer immediately to the customer explaining the problem.\n"
    "4. Once you have generated all the quotes for all the individual items, compare all the estimated delivery dates (from the quotes data) and identify the latest delivery date."
    "   - If the latest delivery date is before or at the customer's requested delivery date, then the order can be fulfilled and you can proceed. Otherwise answer to the customer explaining why the order cannot be fulfilled.\n"
    ...
    "6. For each item in the order, do:\n"
    "   - Call the `handle_sale` to finalize the sales transaction via the Sales Agent. Use the quoted total price for the item-quantity pair that was provided by the Quoting Agent and the the latest delivery date that you got on a previous step.\n"
    "   - Check the request status and messages produced by the Sales Agent\n"
    "   - If the request was declined or there are errors in the messages then answer immediately to the customer explaining the problem.\n"
    "\n"
    "### OUTPUT AND TONE:\n"
...
def _build_orchestrator_agent() -> Agent:
    ...
    def process_order_info(ctx: RunContext[OrchestratorDependencies], customer_message: str) -> CustomerRequestDetails:
        ...
        try:
            ...
            logging.error(f"Error processing customer order.", exc_info=True, extra={"customer_message": customer_message})
            return CustomerRequestDetails(
                messages=["I'm sorry, we encountered an error processing your order. Please try again later or contact customer service."],
                delivery_date=None,
                request_metadata=None
            )

    @orchestrator_agent.tool
    def generate_quote(ctx: RunContext[OrchestratorDependencies], item_name: str, quantity: int, delivery_date: str) -> QuotingAgentOutput:
        """
        Communicate with the Quoting Agent to generate a quote of a single item of a customer request.
        Args:
            ctx: Context about the current call
            item_name: The name of the item that a customer wants to order.
            quantity: The desired quantity to order
            delivery_date: The desired delivery date for the order, in ISO format.

        Returns:
            The quote details that the Quoting Agent generated.
        """
        try:
            logging.info(
                "Getting quote for item.",
                extra={
                    "item_name": item_name,
                    "quantity": quantity,
            ...
            logging.error(f"Error producing quote.", exc_info=True, extra={"item_name": item_name, "quantity": quantity, "order_date": ctx.deps.initial_date})
            return QuotingAgentOutput(
                ...
                quote_calculation=None,
                quote_explanation=None,
                estimated_delivery_date=None
            )

    @orchestrator_agent.tool
    def manage_inventory(ctx: RunContext[OrchestratorDependencies], items: Dict[str, int], delivery_date: str) -> InventoryAgentOutput:
        """
        Communicate with the Inventory Agent to manage inventory for an order and replenish supplies if needed
        Args:
            ctx: Context about the current call
            items: The customer order items
            delivery_date: The desired delivery date for the order, in ISO format.
        ...
        try:
            logging.info(
                ...
                extra={"items": items, "delivery_date": delivery_date, "order_date": ctx.deps.initial_date}
            )
            user_prompt = (
            ...
            logging.error(f"Error managing inventory.",
                          exc_info=True,
                          extra={"items": items, "delivery_date": delivery_date, "order_date": ctx.deps.initial_date})
            return InventoryAgentOutput(
                messages=["I'm sorry, we encountered an error managing the inventory for your order. Please try again later or contact customer service."],
            )

    @orchestrator_agent.tool
    def handle_sale(ctx: RunContext[OrchestratorDependencies], item_name: str, quantity: int, quoted_total_price: float, delivery_date: str) -> SalesAgentOutput:
        """
        ...
        Args:
            ...
            item_name: The name of the item that a customer wants to order.
            quantity: The desired quantity to order
            quoted_total_price: The total price for the item and quantity that was quoted to the customer
            delivery_date: The desired delivery date by the customer

        Returns:
            The response that the Sales Agent generated.
        """
        try:
            logging.info(
            ...
            user_prompt = (
                ...
                f"They previously received a quote for {quantity} units of {item_name} at a total price of ${quoted_total_price:.2f}. "
            ...
            logging.error(f"Error finalizing a sales transaction for an order.",
                          exc_info=True,
                          extra={
                              "item_name": item_name,
                              "quantity": quantity,
            ...
            return SalesAgentOutput(
                messages=["I'm sorry, we encountered an error finalizing a sales transaction for your order. Please try again later or contact customer service."],
...
class OrchestratorAgent:
    ...
    def process_customer_order(self, customer_message) -> OrchestratorAgentOutput:
        ...
        try:
            logging.info(f"Processing customer order: {customer_message}")
            response = self.orchestrator_agent.run_sync(user_prompt=customer_message, deps=self.orchestrator_dependencies)
